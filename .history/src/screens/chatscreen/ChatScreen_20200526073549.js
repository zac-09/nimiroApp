...
class ChatScreen extends React.Component {
	...
	existSameSentMessage = (messages, newMessage) => {
		for (let i = 0; i < messages.length; i++) {
			...
			if (newMessage._id == temp._id && temp.text == newMessage.text && temp.user == newMessage.user) {
				return true;
			}
		}
	...
	onThreadsCollectionUpdate = (querySnapshot) => {
		...
		try {
			...
			console.log('an error occured while loading messages from firetore', error);
		}
	...
	updateCurrentChannel = async (lastMessage) => {
		try {
			await firebase.firestore().collection('channels').doc(this.state.channel.id).update({
				lastMessage: lastMessage,
				lastMessageDate: new Date(),
			});
		} catch (error) {
			...
			>
				<View style={{ flex: 1, height: HEIGHT }}>
					...
					<Container source={bg2}>
						...
						/>
						{Platform.OS === 'android' && (
							<KeyboardAvoidingView behavior="padding" keyboardVerticalOffset={35} />
						)}
